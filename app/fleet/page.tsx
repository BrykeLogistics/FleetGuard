...
function TruckCard({ truck, editingId, showForm, onOpen, onEdit, onDelete, onInspect }: any) {
  ...
  return (
    <div className="card" style={{ padding:'16px', cursor:'pointer', border:isEditing?'1.5px solid #185FA5':undefined }} onClick={() => !showForm&&onOpen(truck)}>
      ...
      {truck.csa&&<div style={{ fontSize:11, color:'#888', marginBottom:3 }}>📍 CSA {truck.csa}</div>}
      ...
      {trucks.map((truck: any, i: number) => {
        ...
        return (
          <div key={truck.id} style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr auto auto', padding:'12px 16px', borderBottom:i<trucks.length-1?'0.5px solid rgba(0,0,0,0.07)':'none', alignItems:'center', cursor:'pointer', background:'white', transition:'background 0.1s' }}
            ...
            <div style={{ fontSize:12, color:'#888' }}>{truck.csa&&<span style={{ color:'#185FA5' }}>CSA {truck.csa} · </span>}{last?new Date(last.created_at).toLocaleDateString():'Never'}</div>
            <span className={`badge ${status.cls}`}>{status.label}</span>
...
function TruckForm({ form, setForm, onSubmit, onCancel, saving, editingId, vinLoading, vinMessage, csaList, onLookupVin, isOwner }: FormProps) {
    ...
    <div className="card" style={{ padding:'20px', marginBottom:16, border:editingId?'1.5px solid #185FA5':undefined }}>
      ...
      <form onSubmit={onSubmit}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:12 }}>
          <div><label>Truck / unit number *</label><input value={form.truck_number} onChange={e => setForm({...form, truck_number:e.target.value})} placeholder="e.g. 501523" required /></div>
          ...
          <div><label>CSA / Location</label><input value={form.csa} onChange={e => setForm({...form, csa:e.target.value})} placeholder="e.g. 326" list="csa-list" /><datalist id="csa-list">{csaList.map(c => <option key={c} value={c} />)}</datalist></div>
...
function FleetPage() {
  ...
  async function loadTrucks() {
    ...
    // Managers only see their CSA
    if (isManager && !isOwner) query = query.eq('csa', profile.csa)
    ...
    if (truckId && data) { const t = data.find((t: any) => t.id === truckId); if (t) openTruck(t) }
    const filter = searchParams.get('filter')
    if (filter) setActiveFilter(filter)
  }

  async function openTruck(truck: any) {
    setSelectedTruck(truck); setLoadingDetail(true)
  ...
  async function lookupVin() {
    ...
    try {
      ...
      const body=get('Body Class').toLowerCase(), gvwr=get('Gross Vehicle Weight Rating From')
      ...
      let vehicle_type=''
      const allText=`${body} ${model.toLowerCase()} ${mfr} ${trim} ${series}`
      const stepVanWMI=['1F6','5F6','1GW','1GX','5GW']
      const stepKw=['p700','p800','p900','p1000','p1100','p1200','step van','stepvan','walk-in','walk in','mt45','mt55','mt35','workhorse','p-series','utilimaster','grumman','olson','hackney']
      const sprinterKw=['sprinter','transit connect','transit','promaster','nv cargo','nv200','express cargo','savana cargo','econoline cargo']
      const boxKw=['m2','npr','nqr','ftr','fvr','f650','f750','c7500','c6500','box truck','straight truck','cab-over','cabover','low cab']
      if (stepVanWMI.includes(wmi)||stepKw.some(k=>allText.includes(k))) vehicle_type='stepvan'
      else if (sprinterKw.some(k=>allText.includes(k))||(body.includes('cargo van')&&gvwrNum<14000)) vehicle_type='sprinter'
      else if (boxKw.some(k=>allText.includes(k))||body.includes('straight')||gvwrNum>=26000) vehicle_type='boxtruck'
      ...
      const fordP: Record<string,string>={'F3K':'P600','F4K':'P700','F4A':'P700','F5K':'P1000','F5A':'P1000','F5F':'P1000','F6K':'P1100','F6A':'P1100','F7K':'P1200','F7A':'P1200','F8K':'P1400','F8A':'P1400'}
      const isPSeries=(wmi==='1F6'||wmi==='5F6')&&!!fordP[vds57]
      const finalModel=isPSeries?fordP[vds57]:(model!=='Not Applicable'?model:'')
      ...
      if (make&&make!=='Not Applicable') {
        ...
        setVinMessage(`✓ Found: ${year} ${finalMake} ${finalModel}${typeLabel?` · ${typeLabel}`:''}${isPSeries?' (update make if Grumman, Morgan Olson, etc.)':''} — review and confirm below`)
  ...
  async function saveTruck(e: React.FormEvent) {
    ...
    if (editingId) {
      await supabase.from('trucks').update(payload).eq('id', editingId); setEditingId(null)
    } else {
      await supabase.from('trucks').insert({ ...payload, company_id: profile?.company_id })
    }
            ...
            <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', flexWrap:'wrap', gap:12 }}>
              <div>
                <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap', marginBottom:4 }}>
                  ...
                  {selectedTruck.csa&&<span style={{ background:'#E6F1FB', color:'#0C447C', fontSize:11, fontWeight:500, padding:'2px 8px', borderRadius:20 }}>📍 CSA {selectedTruck.csa}</span>}
                </div>
      ...
      <div style={{ maxWidth:900, margin:'0 auto', padding:'24px 16px' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16, flexWrap:'wrap', gap:10 }}>
          <div>
            ...
            <div style={{ fontSize:13, color:'#888', marginTop:2 }}>{trucks.length} total · {ownedTrucks.length} owned · {rentalTrucks.length} rental{isManager&&!isOwner?` · CSA ${profile?.csa}`:''}</div>
          </div>
          <div style={{ display:'flex', gap:8, alignItems:'center' }}>
            ...
            {isOwner && <button className="btn btn-primary" onClick={() => { setShowAdd(!showAdd); setEditingId(null); setForm(emptyForm) }}>+ Add truck</button>}
          </div>
        </div>
