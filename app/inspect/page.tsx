  ...
  return (
    <div>
      ...
      <style>{`
        ...
        .thumb-remove{ position:absolute; top:4px; right:4px; width:28px; height:28px; border-radius:50%; background:rgba(0,0,0,0.65); color:white; border:none; cursor:pointer; font-size:15px; display:flex; align-items:center; justify-content:center; -webkit-tap-highlight-color:transparent; }
        .capture-option{ border-radius:12px; padding:16px; margin-bottom:10px; cursor:pointer; -webkit-tap-highlight-color:transparent; display:block; width:100%; box-sizing:border-box; }
          ...
          <div className="card" style={{ padding:'20px 16px' }}>
            {showGuided ? (
              ...
              <div>
                ...
                <div className="capture-option" style={{ border:'2px solid #185FA5', background:'#E6F1FB' }} onClick={() => setShowGuided(true)}>
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                    <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                      <div style={{ fontSize:28 }}>📐</div>
                      <div>
                        <div style={{ fontSize:14, fontWeight:600, color:'#0C447C' }}>Guided capture <span style={{ fontSize:11, background:'#185FA5', color:'white', padding:'2px 8px', borderRadius:10 }}>Recommended</span></div>
                    ...
                    {videoSubMode === 'record' && (
                      <div>
                        <div style={{ position:'relative', background:'#000', borderRadius:12, overflow:'hidden', marginBottom:12, minHeight:220 }}>
                          <video ref={recordingRef} autoPlay playsInline muted style={{ width:'100%', maxHeight:320, objectFit:'cover', display:'block' }} />
                          {isRecording && (
                            <>
                              ...
                              <div style={{ position:'absolute', bottom:12, left:0, right:0, textAlign:'center' }}>
                                <div style={{ display:'inline-block', background:'rgba(0,0,0,0.5)', color:'rgba(255,255,255,0.75)', fontSize:12, padding:'4px 12px', borderRadius:16 }}>Walk around covering all sides</div>
                              </div>
                            </>
                          )}
                        </div>
                        <div style={{ display:'flex', gap:8 }}>
                          {isRecording
                            ? <button onClick={stopRecording} style={{ flex:1, padding:'14px', background:'#E24B4A', color:'white', border:'none', borderRadius:10, fontSize:15, fontWeight:700, cursor:'pointer' }}>⏹ Stop — {formatTime(recordingSeconds)}</button>
                            : <button onClick={startRecording} style={{ flex:1, padding:'14px', background:'#185FA5', color:'white', border:'none', borderRadius:10, fontSize:15, fontWeight:700, cursor:'pointer' }}>🔴 Start recording</button>
                          }
                ...
                {analyzing && (
                  <div style={{ textAlign:'center', padding:'32px 16px', marginTop:8 }}>
                    ...
                    <div key={analyzeStatus} className="analyze-status" style={{ fontSize:13, color:'#555', fontWeight:500 }}>{analyzeStatus}</div>
                    <div style={{ fontSize:11, color:'#bbb', marginTop:6 }}>AI is inspecting your photos</div>
                  </div>
                )}
              </div>
            )}
