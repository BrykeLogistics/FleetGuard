    ...
    <div style={{ background: c.bg, border: `0.5px solid ${c.border}`, borderRadius: 10, padding: '12px 16px', marginBottom: 10 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
        ...
        <Link href={href} style={{
          fontSize: 12, fontWeight: 500, color: c.title, textDecoration: 'none',
          border: `0.5px solid ${c.btn}`, padding: '6px 12px',
...
export default function Dashboard() {
  const { profile, isDriver, isOwner, loading: profileLoading } = useProfile()
  ...
  // Wait for profile before loading so isDriver is correct
  useEffect(() => {
    if (!profileLoading) loadDashboard()
  }, [profileLoading])
  ...
  const condBadge = (c: string) =>
    c === 'Good' ? 'badge-green' : (c === 'Critical' || c === 'Poor') ? 'badge-red' : c === 'Fair' ? 'badge-amber' : 'badge-gray'
    ...
    <div>
      ...
      <div style={{ maxWidth: 500, margin: '0 auto', padding: '32px 20px 24px', textAlign: 'center' }}>
        ...
        <Link href="/inspect" className="btn btn-primary" style={{
          ...
          display: 'block', textDecoration: 'none', width: '100%', maxWidth: 320, margin: '0 auto',
        }}>
          Start inspection
        </Link>
      </div>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '20px 16px 40px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10, marginBottom: 20 }}>
          {[
            ...
            <Link key={s.label} href={s.href} style={{
              ...
              border: '0.5px solid rgba(0,0,0,0.08)', textDecoration: 'none', color: 'inherit', display: 'block',
            }}>
        ...
        <div className="card" style={{ padding: '14px 16px', marginBottom: 16 }}>
          ...
          {loading ? (
            ...
            return (
              <Link key={insp.id} href={`/fleet?truck=${truck?.id}`} style={{
                display: 'block', borderBottom: '0.5px solid rgba(0,0,0,0.07)',
                textDecoration: 'none', color: 'inherit', padding: '11px 0',
              }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <Link href="/inspect" className="btn btn-primary" style={{ textAlign: 'center', textDecoration: 'none', padding: '14px', fontSize: 15, borderRadius: 10 }}>
            + New inspection
          </Link>
          <Link href="/fleet" className="btn" style={{ textAlign: 'center', textDecoration: 'none', padding: '14px', fontSize: 15, borderRadius: 10 }}>
            Manage fleet
          </Link>
        </div>
